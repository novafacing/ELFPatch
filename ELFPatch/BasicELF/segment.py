...
class Segment(object):
    def __init__(self, physical_offset, virtual_address, size, flags=PT_R|PT_W|PT_X, align=0x10, content=""):
        if len(content) > size:
            raise Exception(u"Content is larger than size")
    ...
    def update_data(self, content, start_offset=None, end_offset=None):
        ...
        if end_offset > len(self.content):
            self.content += (end_offset - len(self.content)) * "\x00"
